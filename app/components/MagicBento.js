...
/* MagicBento (React Bits) — integrated for the "What the work produces" section.
   Themed to MAAI (evergreen cards, magenta glow) and populated with our real
   outcomes + imagery. Effects (spotlight, border-glow, particles, tilt, magnetism,
   click ripple) are the original component. */
...
const DEFAULT_GLOW_COLOR = "230, 46, 107"; // MAAI spark magenta (#E62E6B)
...
const CardInner = ({ card }) => (
  <>
    <div className="mb-media" aria-hidden="true">
...
const MagicBento = ({
  ...
  /* mobile-only slider: scroll the card row one card at a time */
  const scrollByCard = (dir) => {
    const el = gridRef.current;
    if (!el) return;
    const card = el.querySelector(".magic-bento-card");
  ...
  return (
    <>
      ...
      <div className="bento-arrows" aria-hidden="true">
        <button type="button" className="bento-arrow" aria-label="Previous industry" onClick={() => scrollByCard(-1)}>
        ...
        <button type="button" className="bento-arrow" aria-label="Next industry" onClick={() => scrollByCard(1)}>
