...
/* CursorGrid (React Bits) — integrated as a full-page ambient background.
   Two integration adaptations from the stock component: the pointer listeners are
   attached to `window` (not the container) so the effect tracks the cursor across
   the whole page while the canvas stays `pointer-events:none` and never blocks the
   UI beneath it. Everything else is the original component. */
...
const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    // window (not container) so the ambient grid tracks the cursor everywhere
